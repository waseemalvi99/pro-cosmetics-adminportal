...
import type { SalesmanDto } from "@/types";
...
export default function SalesmenPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<SalesmanDto | null>(null);

  const { data: response, isLoading } = useQuery({
    queryKey: ["salesmen", page, search],
    queryFn: () => salesmenApi.list({ page, pageSize: 10, search: search || undefined }),
  });

  const deleteMutation = useMutation({
    mutationFn: async (id: number) => {
      const res = await salesmenApi.delete(id);
      if (!res.success) throw new Error(res.message || "Failed to delete salesman");
      return res;
    },
    onSuccess: () => {
      toast.success("Salesman deleted", {
        description: "The salesman has been removed successfully.",
      });
  ...
  const salesmen: SalesmanDto[] = pagedData?.items ?? [];
  ...
  return (
    <div className="space-y-6">
                      ...
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem
                              variant="destructive"
                              onClick={() => setDeleteTarget(salesman)}
                            >
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="font-display">Delete salesman</AlertDialogTitle>
            <AlertDialogDescription>
