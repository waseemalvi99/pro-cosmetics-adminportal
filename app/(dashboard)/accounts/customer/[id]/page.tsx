...
export default function CustomerStatementPage() {
  ...
  const { data: response, isLoading } = useQuery({
    ...
    queryFn: () => accountsApi.customerStatement(id, { fromDate, toDate }),
  ...
  const handleExportPdf = async () => {
    try {
      await accountsApi.downloadCustomerStatementPdf(id, { fromDate, toDate });
  ...
  if (Number.isNaN(id)) {
    return (
      <div className="space-y-6">
        <PageHeader title="Customer Statement" description="Invalid customer ID" />
      </div>
    );
  }
            ...
            <>
              <div className="mb-6 grid gap-4 sm:grid-cols-4">
                ...
                <div className="rounded-lg border p-4">
                  ...
                  <p className="mt-1 font-display text-xl font-bold text-red-600 dark:text-red-400">{formatCurrency(statement.totalDebits)}</p>
                </div>
                <div className="rounded-lg border p-4">
                  <p className="text-xs font-medium text-muted-foreground">Total Credits</p>
                  <p className="mt-1 font-display text-xl font-bold text-emerald-600 dark:text-emerald-400">{formatCurrency(statement.totalCredits)}</p>
                </div>
